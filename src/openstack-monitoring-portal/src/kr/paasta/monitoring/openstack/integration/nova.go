...
func GetNova(openstackProvider models.OpenstackProvider, provider *gophercloud.ProviderClient) *Nova {
	return &Nova{
...
func (n *Nova) GetOpenstackResources() (result models.HypervisorResources, err error){
	...
	response, err := client.Get(fmt.Sprintf("%s/%s/%s/os-hypervisors/statistics", models.NovaUrl, models.NovaVersion, models.DefaultProjectId), nil, nil)
 ...
 */
func (n *Nova) GetComputeNodeResources() (result []models.NodeResources, err error){
...
*/
	...
	response, err := client.Get(fmt.Sprintf("%s/%s/%s/os-hypervisors/detail", models.NovaUrl, models.NovaVersion, models.DefaultProjectId), nil, nil)
	...
	if(len(resources) > 0){
		...
		for _, compute_node :=range resources {
			...
			var node_info models.NodeResources
 ...
 */
func (n *Nova) GetProjectResourcesLimit(project_id string) (result models.ProjectResourcesLimit, err error){
	var data interface{}
	/*provider, err := utils.GetAdminToken(n.OpenstackProvider)
	if err != nil {
		return result, err
	...
	response, err := client.Get(fmt.Sprintf("%s/%s/%s/os-quota-sets/%s", models.NovaUrl, models.NovaVersion, models.DefaultProjectId, project_id), nil, nil)
	if err != nil {
		return result, err
	}
...
func (n *Nova) GetProjectInstancesList(apiRequest models.ProjectReq) (result []models.InstanceInfo, err error){
	...
	if apiRequest.Marker == ""{
		...
		if apiRequest.HostName == ""{
			response, apiError = client.Get(fmt.Sprintf("%s/%s/%s/servers/detail?all_tenants=1&limit=%s&project_id=%s",
				   models.NovaUrl, models.NovaVersion, models.DefaultProjectId, apiRequest.Limit, apiRequest.ProjectId), nil, nil)
		}else{
			response, apiError = client.Get(fmt.Sprintf("%s/%s/%s/servers/detail?all_tenants=1&limit=%s&hostname=%s&project_id=%s",
				  models.NovaUrl, models.NovaVersion, models.DefaultProjectId , apiRequest.Limit, apiRequest.HostName, apiRequest.ProjectId), nil, nil)
		}
		...
		if apiRequest.HostName == ""{
			response, apiError = client.Get(fmt.Sprintf("%s/%s/%s/servers/detail?all_tenants=1&limit=%s&marker=%s&project_id=%s",
				models.NovaUrl, models.NovaVersion, models.DefaultProjectId, apiRequest.Limit, apiRequest.Marker, apiRequest.ProjectId), nil, nil)
		}else{
			response, apiError = client.Get(fmt.Sprintf("%s/%s/%s/servers/detail?all_tenants=1&limit=%s&marker=%s&hostname=%s&project_id=%s",
				models.NovaUrl, models.NovaVersion, models.DefaultProjectId, apiRequest.Limit, apiRequest.Marker, apiRequest.HostName, apiRequest.ProjectId), nil, nil)
		}

	}

	//fmt.Printf("%s/%s/%s/server/details/%s?limit=5", models.NovaUrl, models.NovaVersion, models.DefaultProjectId, project_id)
	if apiError != nil {
 ...
 */
func (n *Nova) GetProjectInstances(project_id string) (result []models.InstanceInfo, err error){
	...
	response, err := client.Get(fmt.Sprintf("%s/%s/%s/os-simple-tenant-usage/%s", models.NovaUrl, models.NovaVersion, models.DefaultProjectId, project_id), nil, nil)
 ...
 */
func (n *Nova) GetInstanceDetail(instance_id, project_id string) (result models.InstanceDetail, err error){
	var data interface{}
	...
	response, err := client.Get(fmt.Sprintf("%s/%s/%s/servers/%s", models.NovaUrl, models.NovaVersion, project_id, instance_id), nil, nil)
	if err != nil {
		return result, err
	}
	rawdata, err := ioutil.ReadAll(response.Body)
	...
	if len(address_info) > 0 {
		instance_network := make([]models.NetworkInfo, len(address_info["private_net"].([]interface{})))
		var index int
	...
	response, err = client.Get(fmt.Sprintf("%s/%s/%s/flavors/%s", models.NovaUrl, models.NovaVersion, project_id, flavor_info["id"]), nil, nil)
	if err != nil {
		return result, err
	}
	rawdata, err = ioutil.ReadAll(response.Body)
	...
	response, err = client.Get(fmt.Sprintf("%s/%s/%s/images/%s", models.NovaUrl, models.NovaVersion, project_id, image_info["id"].(string)), nil, nil)
	if err != nil {
		return result, err
	}
	rawdata, err = ioutil.ReadAll(response.Body)

(function() {
  ...
  angular
    .module('monitoring')
    .directive('liquid', liquid);

  /** @ngInject */
  function liquid($timeout, $window){
    return {
      restrict: 'E',
      scope: {
        seq: '=',
      ...
      template: '<svg id="fillGauge{{seq}}" width="97%"></svg>',

      link: function(scope, elem) {
        var gaugeGroup;

        $timeout(function() {
          scope.gauge1 = loadLiquidFillGauge('fillGauge'+scope.seq, 50);
        ...
        function loadLiquidFillGauge (elementId, value, config) {
          ...
          function GaugeUpdater(){
            this.update = function(value){
              if(value > 100) {
                gaugeGroup.selectAll('path').style('fill','#ff3333');
                gaugeGroup.selectAll('circle').style('fill','#ffbbbb');
                gaugeGroup.selectAll('text').style('fill','#ff0000');
              }

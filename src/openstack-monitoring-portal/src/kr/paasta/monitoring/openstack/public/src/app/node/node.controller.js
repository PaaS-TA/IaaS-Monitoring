(function() {
  ...
  function ManageNodeController($scope, $timeout, $window, $sce, $exceptionHandler, manageNodeService) {
    ...
    (vm.getManageNodeSummary = function() {
      if(vm.loadingSummary != undefined) vm.loadingSummary = true;
    ...
    vm.scope.getTopProcessByHostname = function(obj) {
      ...
      oldObj['select'] = '';
      obj['select'] = 'active';
      vm.selectedManageNode = $sce.trustAsHtml('[ <span style="color:#337ab7;">' + obj.hostname + '</span> ]');
  ...
  /** @ngInject */
  function ComputeNodeController($scope, $timeout, $window, $sce, $exceptionHandler, mainService, computeNodeService) {
    var vm = this;
    vm.scope = $scope;
    vm.Math = Math;
    ...
    vm.scope.getTopProcessByHostname = function(obj) {
      ...
      oldObj['select'] = '';
      obj['select'] = 'active';
      vm.selectedComputeNode = $sce.trustAsHtml('[ <span style="color:#337ab7;">' + obj.hostname + '</span> ]');
      vm.scope.getTopProcessCpuList(obj.hostname);
      vm.scope.getTopProcessMemoryList(obj.hostname);
      oldObj = obj;
    };
  ...
  /** @ngInject */
  function NodeDetailController($scope, $log, $stateParams, $interval, $timeout, $rootScope,
                                    computeNodeService, logService, nodeChartConfig, common, cookies, nvd3Generator, $exceptionHandler, anchorSmoothScroll) {
    var vm = this;
    vm.scope = $scope;
    vm.Math = Math;
    ...
    vm.scope.defaultRecentLog = function(message, optParam) {
      ...
      logService.dtvDefaultRecentLog(param).then(
        function(result) {
          vm.scope.recentLogs = result.data.messages;

          vm.scope.optLogstashIndex = result.data.logstashIndex;
          vm.scope.optStartTime = result.data.startTime;
          vm.scope.optEndTime = result.data.endTime;
    ...
    vm.scope.specificTimeRangeLog = function(message, optParam) {
      ...
      logService.dtvSpecificTimeRangeLog(param).then(
        function(result) {
          vm.scope.recentLogs = result.data.messages;
    ...
    vm.scope.pageChanged = function() {
      anchorSmoothScroll.scrollTo('logs');
      var optParam = {
